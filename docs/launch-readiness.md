...
- 已加入 MongoDB 驱动、连接管理和核心集合索引初始化；登录和用户查询已接入用户仓储，衣物、模特、AI 任务和额度流水仍需继续迁移。
...
2. 生产后端
   ...
   - 将 clothing_items、user_photos、ai_tasks、credit_logs 的业务读写从内存 store 迁移到 MongoDB/PostgreSQL 仓储层。
